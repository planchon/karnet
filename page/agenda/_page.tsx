...
import type { DayHeaderContentArg, EventContentArg, SlotLabelContentArg } from "@fullcalendar/core/index.js";
        ...
        <div className="h-full w-full">
            <div className="flex h-10 w-full items-center justify-between border-b">
                ...
                <div className="flex h-full flex-row items-center justify-center gap-2 pr-4">
                    ...
                    >
                        <Button onClick={() => commands.toggleEvent()} size="sm" variant="ghost">
    ...
    return (
        <div className="calendar-container h-full w-full">
            <FullCalendar
                ...
                eventContent={(arg) => <Event arg={arg} />}
                events={events}
                firstDay={1}
                headerToolbar={false}
                height="100%"
                    ...
                    <div className="flex min-w-0 flex-col gap-0.5">
                        <div data-line-count="6">
                            <div className="line-clamp-6 break-words font-medium">
                                ...
                                <span className="overflow-hidden text-ellipsis" data-state="closed">
                                    {arg.event.title}
                                </span>
                            </div>
                        </div>
...
const SlotLabel = ({ arg }: { arg: SlotLabelContentArg }) => (
    <div className="w-18 select-none">
        <span className="font-medium text-muted-foreground text-xs">{arg.text}</span>
    </div>
);

const DayHeader = ({ arg }: { arg: DayHeaderContentArg }) => (
    <div className="select-none py-[5.5px]">
        <span className={cn("font-medium text-muted-foreground text-xs", arg.isToday && "font-semibold")}>
            {arg.text}
        </span>
    </div>
);
