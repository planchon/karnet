...
import { BookIcon, ChevronDownIcon, LucideLink } from "lucide-react";
...
export const SourcesTrigger = ({ className, count, children, ...props }: SourcesTriggerProps) => (
    <CollapsibleTrigger className={cn("flex items-center gap-2 pt-1", className)} {...props}>
        {children ?? (
            <>
                <LucideLink className="h-4 w-4" />
                <p className="font-medium">{count} sources used</p>
    ...
    return (
        <a className="flex items-center gap-2" href={href} rel="noreferrer" target="_blank" {...props}>
            {children ?? (
                <>
                    {favicon && (
                        <img alt="Favicon" className="h-4 w-4 rounded-full" height={16} src={favicon} width={16} />
                    )}
                    {!favicon && <BookIcon className="h-4 w-4" />}
