...
export const Grid = ({ x, y, z, size }: { x: number; y: number; z: number; size: number }) => {
                ...
                {gridSteps.map(({ min, mid, step }, i) => {
                    ...
                    return (
                        <pattern
                            ...
                            width={s}
                        >
                            <circle className="tl-grid-dot" cx={gxo} cy={gyo} opacity={opacity} r={1} />
                        </pattern>
                    );
                })}
            </defs>
            {gridSteps.map(({ step }, i) => (
                <rect fill={`url(#${id}_${step})`} height="100%" key={i} width="100%" />
            ))}
        </svg>
    );
};
