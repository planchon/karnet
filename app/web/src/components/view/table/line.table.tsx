			...
			<ContextMenu>
				<ContextMenuTrigger>
					<Link
						...
						className={cn(
							"flex z-10 h-10 w-full select-none hover:cursor-pointer items-center justify-between pl-3 pr-5 py-2 focus:outline-none",
...
export const ViewItemCheckbox = () => {
	return (
		<div className="flex z-[1000] size-8 group items-center justify-center transition-all duration-300">
			<Checkbox
				onClick={(e) => {
					e.stopPropagation();
				}}
				className="size-4 hidden group-hover:block group-hover:border-accent-foreground/50"
			/>
		</div>
	);
};
